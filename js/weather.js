...
var getWeather = () => {
  ...
  // 구글 geocode api
  lib.ajaxSubmit('https://maps.googleapis.com/maps/api/geocode/json'+params, 'GET', '', function(data){
    // 값이 정상적인 경우
    if(data.status === 'OK'){
      ...
      lib.ajaxSubmit(url, 'GET', '', function(data){
        ...
        for(var i=0; i<$(data).find('data').length; i++){
          ...
          if($item.find('day').text() === '2')
            break;
          content = $("<li>").appendTo(weather);
          $('<div>'+$item.find('hour').text()+":00</div>").appendTo(content);
          ...
          switch($item.find('wfEn').text()){
            ...
            default:
              $("<div>no image ( "+$item.find('wfEn').text()+" )</div>").appendTo(content);
          }
          $('<div>'+$item.find('temp').text()+"°</div>").appendTo(content);
          $item.find('wfEn').text()
        };
      });
    } else {
...
var xyToGrid = (v1, v2) => {
  ...
  return "http://www.kma.go.kr/wid/queryDFS.jsp?gridx="+rs["x"]+"&gridy="+rs["y"]
}
